...
class payrollreportexcelwiz(models.TransientModel):
    ...
    struct_id = fields.Many2one('hr.payroll.structure', string="Structure Name")
    company = fields.Many2one('res.company', default=lambda self: self.env['res.company']._company_default_get(),
                              string="Company")

    # tag_id = fields.Many2one('hr.employee.category', string='Project Name')
    # to get salary rules names
    # @api.multi
    def get_rules(self):
        vals = []

        heads = self.env['hr.salary.rule'].search([('active', 'in', (True)),('struct_id', '=', self.struct_id.id)], order='sequence asc')
        list = []
        for head in heads:
    ...
    def get_item_data(self):
        ...
        if self.from_date and self.date_end:
            ...
            worksheet.write(row, 1, 'Bank Name', cell_text_format)
            worksheet.write(row, 2, 'Employee Account Number', cell_text_format)
            worksheet.write(row, 3, 'Employee PaySlip', cell_text_format)
            ...
            payslip_ids = self.env['hr.payslip'].search(
                [('date_from', '=', self.from_date), ('date_to', '=', self.date_end), ('state', '=', 'verify')])
            if payslip_ids:

                for payslip in payslip_ids:
                    name = payslip.employee_id.name
                    reference= payslip.number
                    id = payslip.employee_id.bank_account_id.bank_id.name
                    tag = payslip.employee_id.bank_account_id.acc_number

                    worksheet.write(ro, col, name or '', cell_text_format_new)
                    worksheet.write(ro, col + 1, id or '', cell_text_format_new)
                    worksheet.write(ro, col + 2, tag or '', cell_text_format_new)
                    worksheet.write(ro, col + 3, reference or '', cell_text_format_new)
        ...
        return {
            ...
            'url': "web/content/?model=ir.attachment&id=" + str(attachment_id.id) +
                   "&filename=payroll.xlsx&field=datas&download=true&filename=" + attachment_id.store_fname,
            'target': 'self',
        }
